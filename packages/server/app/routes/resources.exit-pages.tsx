...
export async function loader({ context, request }: LoaderFunctionArgs) {
    ...
    if (context.cloudflare.env.DB) {
        try {
            summary = await getExitPageSummary(
            ...
            // Keep the dashboard usable if exit-page SQL fails (e.g. schema drift).
            console.error("getExitPageSummary failed", err);
...
export const ExitPagesCard = ({
    ...
    return (
        <PaginatedTableCard
            siteId={siteId}
            interval={interval}
            columnHeaders={["Exit Page", "Sessions", "Views"]}
            dataFetcher={useFetcher<typeof loader>()}
            filters={filters}
            loaderUrl="/resources/exit-pages"
            onClick={(path) => {
